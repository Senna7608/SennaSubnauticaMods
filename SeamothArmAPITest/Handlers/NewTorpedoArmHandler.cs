...
using SeamothArms.API.ArmHandlers;
using SeamothArmAPITest.Prefabs;
using SeamothArms.API.Interfaces;

namespace SeamothArmAPITest.Handlers
{
    internal class NewTorpedoArmHandler : TorpedoArmHandler, ISeamothArmHandler
    {
        ...
        GameObject ISeamothArmHandler.GetGameObject()
        {
            return gameObject;
        }
        ...
        private IEnumerator GetItemsContainer(string slotName)
        {
            ...
            {
                container = ThisSeamoth.GetStorageInSlot(ThisSeamoth.GetSlotIndex(slotName), NewTorpedoArmPrefab.TechTypeID);
        ...
        void ISeamothArmHandler.SetSide(SeamothArm arm)
        {
            if (container != null)
            {
                container.onAddItem -= OnAddItem;
        ...
        bool ISeamothArmHandler.OnUseDown(out float cooldownDuration)
        {
            return TryShoot(out cooldownDuration, true);
        }


        bool ISeamothArmHandler.OnUseHeld(out float cooldownDuration)
        {
            return TryShoot(out cooldownDuration, false);
        }


        bool ISeamothArmHandler.OnUseUp(out float cooldownDuration)
        {
        ...
        bool ISeamothArmHandler.OnAltDown()
        {
            return false;
        }

        void ISeamothArmHandler.Update(ref Quaternion aimDirection)
        {
        }


        void ISeamothArmHandler.Reset()
        {
            animator.SetBool("use_tool", false);
        }
        ...
        bool ISeamothArmHandler.HasClaw()
        {
            return false;
        }

        bool ISeamothArmHandler.HasDrill()
        {
            return false;
        }

        void ISeamothArmHandler.SetRotation(SeamothArm arm, bool isDocked)
        {
            if (ThisSeamoth == null)
            {
                return;
        ...
        float ISeamothArmHandler.GetEnergyCost()
        {
            return 0;
        }
