        ...
        private void SearchForModdedTechTypes()
        {
            ...
            for (int j = 0; j < techTypeArray.Length; j++)
            {
                if (techTypeArray[j] >= 11000)
                {
                    ...
                    SNLogger.Log("ModdedTechTypeHelper", $"Modded techtype found! Name: [{techName}], ID: [{(int)techType}], Type: [{equipmentType}]");
                    i++;
                }
            }

            SNLogger.Log("ModdedTechTypeHelper", $"Found [{i}] modded TechType(s).");
        }
