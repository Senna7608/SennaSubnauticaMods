...
namespace Common.GUIHelper
{
    public static class SNHorizontalSlider
    {
        public static void CreateHorizontalSlider(Rect rect, ref float sliderValue, float leftValue, float rightValue, string label, Event<object> onSliderValueChangedEvent)
        {
            ...
            GUI.Label(new Rect(rect.x + labelSize.x + 5, rect.y + 5, rect.width - labelSize.x, labelSize.y), string.Format("{0:0.00}", sliderValue), SNStyles.GetGuiItemStyle(GuiItemType.LABEL, GuiColor.Green, textAnchor: TextAnchor.MiddleLeft));
